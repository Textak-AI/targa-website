...
/* ═══ PREMIUM BACKGROUND ═══ */
const TARGA_TILE = "data:image/svg+xml," + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="96" height="104" viewBox="0 0 96 104"><polygon points="36.6 36.2 30.4 36.2 24 29.4 17.2 36.2 11 36.2 19.6 13.8 21.8 8.4 24 4.2 36.6 36.2" fill="none" stroke="white" stroke-width="0.5" opacity="0.05"/><polygon points="28.2 30.2 19.6 30.2 24 21.2 28.2 30.2" fill="none" stroke="white" stroke-width="0.4" opacity="0.035"/><polygon points="84.6 88.2 78.4 88.2 72 81.4 65.2 88.2 59 88.2 67.6 65.8 69.8 60.4 72 56.2 84.6 88.2" fill="none" stroke="white" stroke-width="0.5" opacity="0.05"/><polygon points="76.2 82.2 67.6 82.2 72 73.2 76.2 82.2" fill="none" stroke="white" stroke-width="0.4" opacity="0.035"/></svg>');

/* ═══ CANVAS-BASED TARGA PATTERN WITH PER-ICON SWEEP ═══ */
function TargaCanvasPattern() {
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      const sweepPos = sweepRef.current;
      ...
      for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < cols; col++) {
          const x = col * cellW + (isOdd ? cellW * 0.5 : 0);
          const y = row * cellH;
...
function PremiumBg({ children, orb1 = "rgba(139,92,246,0.07)", orb2 = "rgba(14,178,175,0.06)", style: sx = {} }) {
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", overflow: "hidden", background: "#080e1a", ...sx }}>
      ...
      {/* Orb 1 */}
      <div style={{ position: "absolute", top: "20%", left: "20%", width: 500, height: 500, borderRadius: "50%", background: orb1, filter: "blur(120px)", pointerEvents: "none" }} />
      {/* Orb 2 */}
      <div style={{ position: "absolute", bottom: "20%", right: "20%", width: 500, height: 500, borderRadius: "50%", background: orb2, filter: "blur(120px)", pointerEvents: "none" }} />
      {/* Content */}
...
function GlowCard({ children, glowColor = "rgba(14,178,175,0.15)", style: sx = {} }) {
  const [h, setH] = useState(false);
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{ position: "relative", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12, padding: "32px 28px", transition: "all 0.4s", transform: h ? "translateY(-4px)" : "none", boxShadow: h ? "0 20px 48px rgba(0,0,0,0.3)" : "0 4px 16px rgba(0,0,0,0.15)", ...sx }}>
      {/* Hover glow */}
      <div style={{ position: "absolute", inset: -1, borderRadius: 12, background: glowColor, opacity: h ? 1 : 0, transition: "opacity 0.4s", filter: "blur(24px)", pointerEvents: "none", zIndex: 0 }} />
      <div style={{ position: "relative", zIndex: 1 }}>{children}</div>
    </div>
  );
}
...
/* ═══ PARALLAX WRAPPER ═══ */
function ParallaxWrap({ children, offset = 40 }) {
  const ref = useRef(null);
  ...
  return (
    <div ref={ref} style={{ transform: "translateY(" + y + "px)", transition: "transform 0.1s linear", willChange: "transform" }}>
      {children}
    </div>
  );
}
...
function Nav({ page, setPage }) {
  ...
  const Link = ({ children, to }) => {
    ...
    const color = (h || active ? C.white : "rgba(255,255,255,0.55)");
  ...
  const burgerColor = C.white;
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: scrolled ? "8px 0" : "16px 0", background: scrolled ? "rgba(15,32,53,0.85)" : "transparent", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: scrolled ? "1px solid rgba(14,178,175,0.08)" : "1px solid transparent", transition: "all 0.4s" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: mobile ? "0 20px" : "0 40px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div onClick={() => { setPage("home"); window.scrollTo(0, 0); setMenuOpen(false); }}><Logo variant="light" size={mobile ? "sm" : "sm"} /></div>
        {mobile ? (
          <div onClick={() => setMenuOpen(!menuOpen)} style={{ cursor: "pointer", padding: 8, zIndex: 110 }}>
            <div style={{ width: 22, height: 2, background: burgerColor, marginBottom: 5, transition: "all 0.3s", transform: menuOpen ? "translateY(7px) rotate(45deg)" : "none" }} />
            <div style={{ width: 22, height: 2, background: burgerColor, marginBottom: 5, transition: "all 0.3s", opacity: menuOpen ? 0 : 1 }} />
            <div style={{ width: 22, height: 2, background: burgerColor, transition: "all 0.3s", transform: menuOpen ? "translateY(-7px) rotate(-45deg)" : "none" }} />
          </div>
        ) : (
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            ...
            <div style={{ display: "flex", flexDirection: mobile ? "column" : "row", alignItems: mobile ? "flex-start" : "center", gap: mobile ? 16 : 20, marginBottom: mobile ? 20 : 28 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: mobile ? "14px 20px" : 0, alignItems: "center", justifyContent: mobile ? "center" : "space-between", flex: mobile ? undefined : 1, width: mobile ? "100%" : undefined }}>
                {[
    ...
    <>
      ...
      <section style={{ background: C.navyDeep, padding: mobile ? "48px 20px" : "80px 40px" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "repeat(2,1fr)", gap: 28, marginTop: mobile ? 24 : 40 }}>
            {[
              ...
              <Reveal key={p.name} delay={i * 0.1}><GlowCard glowColor={i === 0 ? "rgba(14,178,175,0.1)" : i === 1 ? "rgba(74,159,232,0.08)" : "rgba(251,191,36,0.06)"} style={{ padding: mobile ? 32 : 36, minHeight: mobile ? "auto" : 280 }}>
        ...
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr 1fr", gap: 28, marginTop: mobile ? 24 : 40 }}>
            {[
              ...
              <Reveal key={t} delay={i * 0.1}><GlowCard glowColor={i === 0 ? "rgba(251,191,36,0.08)" : i === 1 ? "rgba(14,178,175,0.1)" : "rgba(139,92,246,0.08)"} style={{ padding: "28px 24px", minHeight: 180 }}>
