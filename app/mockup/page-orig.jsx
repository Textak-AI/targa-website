...
function StatCard(p){return(<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:10,padding:"16px 18px"}}><div style={{fontSize:10,color:C.gray,marginBottom:4}}>{p.label}</div><div style={{fontSize:26,fontWeight:500,color:p.color||C.navy,letterSpacing:"-0.02em"}}>{p.value}</div>{p.sub&&<div style={{fontSize:11,color:p.subColor||C.green,fontWeight:500,marginTop:4}}>{p.sub}</div>}</div>);}
...
".trg-ph-title{font-size:19px}",
".trg-hero{flex-direction:column;align-items:stretch;gap:16px;padding:18px 18px}",
".trg-hero-divider{display:none}",
".trg-hero-metrics{flex-direction:row;min-width:0;gap:20px;padding-top:14px;border-top:1px solid #e6ecf2}",
".trg-hero-metrics > div{flex:1}",
".trg-hero-headline{font-size:17px}",
".trg-hero-num{font-size:22px}",
".trg-focal-title{font-size:20px}",
".trg-focal-num{font-size:17px}",
...
function PH(p){return(<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingBottom:18,borderBottom:"1px solid "+C.border,marginBottom:18,gap:12,flexWrap:"wrap"}}><div style={{minWidth:0}}><div style={{fontSize:10,fontWeight:500,letterSpacing:1.5,color:C.teal,textTransform:"uppercase",marginBottom:2}}>{p.eyebrow}</div><div className="trg-ph-title" style={{fontWeight:500,color:C.navy,fontFamily:"Space Grotesk, sans-serif",letterSpacing:"-0.02em"}}>{p.title}</div></div>{p.right&&<div>{p.right}</div>}</div>);}
...
<div style={{fontSize:10,fontWeight:500,letterSpacing:1.2,color:C.tealDk,textTransform:"uppercase",marginBottom:8,display:"flex",alignItems:"center",gap:6}}><TM s={12}/> This week's focus</div>
...
<div style={{fontSize:13,color:"#4a5868",lineHeight:1.6}}>Three of four goals on track. The COGS initiative is 40% against an $8M target and Mark flagged a supplier blocker. Gross margin hits board review in 11 days.</div>
</div>
<div className="trg-hero-divider" style={{background:C.border}}/>
<div className="trg-hero-metrics">
<div><div style={{fontSize:10,color:C.gray,letterSpacing:0.5,textTransform:"uppercase",fontWeight:500,marginBottom:2}}>Confidence</div><div style={{display:"flex",alignItems:"baseline",gap:6}}><span className="trg-hero-num" style={{fontFamily:"Space Grotesk, sans-serif",fontWeight:500,color:C.navy,letterSpacing:"-0.02em",lineHeight:1}}>69<span style={{fontSize:"0.65em"}}>%</span></span><span style={{fontSize:11,color:C.green,fontWeight:500}}>{"\u2197"} +3</span></div></div>
<div><div style={{fontSize:10,color:C.gray,letterSpacing:0.5,textTransform:"uppercase",fontWeight:500,marginBottom:2}}>Board review</div><div style={{display:"flex",alignItems:"baseline",gap:6}}><span className="trg-hero-num" style={{fontFamily:"Space Grotesk, sans-serif",fontWeight:500,color:C.amber,letterSpacing:"-0.02em",lineHeight:1}}>11<span style={{fontSize:"0.6em"}}>d</span></span><span style={{fontSize:11,color:C.gray}}>Q1 materials</span></div></div>
</div>
</div>
...
<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><SI s={14}/><div style={{flex:1,fontSize:15,fontWeight:500,color:C.navy,lineHeight:1.35}}>{s.t}</div><Av i={s.owner} s={24} bg={s.owner==="JT"?C.gold:s.owner==="MS"?C.red:C.navy} fg={s.owner==="JT"?C.navy:"#fff"}/></div>
<div style={{display:"flex",alignItems:"center",gap:16,marginBottom:10,flexWrap:"wrap"}}>
<div style={{display:"flex",alignItems:"baseline",gap:4}}><span style={{fontFamily:"Space Grotesk, sans-serif",fontSize:18,fontWeight:500,color:C.navy,letterSpacing:"-0.02em",lineHeight:1}}>{s.progress}%</span><span style={{fontSize:10,color:C.gray}}>progress</span></div>
<div style={{width:1,height:14,background:C.border}}/>
<div style={{display:"flex",alignItems:"baseline",gap:4}}><span style={{fontFamily:"Space Grotesk, sans-serif",fontSize:15,fontWeight:500,color:C.navy,letterSpacing:"-0.02em",lineHeight:1}}>{s.impact}</span></div>
...
<div style={{fontSize:10,fontWeight:500,letterSpacing:0.5,color:C.tealDk,textTransform:"uppercase",marginBottom:10}}>This week</div>
<div style={{background:C.white,border:"1px solid "+C.tealBd,borderRadius:8,padding:"10px 12px",marginBottom:6,fontSize:13,color:C.navy,lineHeight:1.55}}>Your team resolved 2 actions and moved 1 item from At Risk to On Track. The COGS blocker on gross margin expansion remains your top priority.</div>
<div style={{background:C.white,border:"1px solid "+C.tealBd,borderRadius:8,padding:"10px 12px",marginBottom:6}}><div style={{fontSize:12,fontWeight:500,color:C.navy,marginBottom:2}}>Pricing approval is overdue</div><div style={{fontSize:11,color:C.gray,lineHeight:1.5}}>Blocking price realization. 3 days past due.</div></div>
<div style={{background:C.white,border:"1px solid "+C.tealBd,borderRadius:8,padding:"10px 12px"}}><div style={{fontSize:12,fontWeight:500,color:C.navy,marginBottom:2}}>OpEx efficiency dropping</div><div style={{fontSize:11,color:C.gray,lineHeight:1.5}}>Confidence fell 7 points. Two vendor blockers.</div></div>
</div>
<Eye><TI s={12}/> Recent activity</Eye>
<div style={{background:C.white,border:"1px solid "+C.border,borderRadius:10,padding:"4px 14px"}}>
{[{i:"MS",bg:C.red,fg:"#fff",n:"Mark S.",a:"flagged blocker on COGS",t:"2h ago"},{i:"JT",bg:C.gold,fg:C.navy,n:"Joe T.",a:"updated margin target",t:"Yesterday"},{i:"SD",bg:C.navy,fg:"#fff",n:"Sarah D.",a:"completed 2 interviews",t:"2d ago"}].map(function(e,idx){return(<div key={idx} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:idx<2?"1px solid #f0f3f6":"none"}}><Av i={e.i} bg={e.bg} fg={e.fg} s={22}/><div style={{fontSize:12,lineHeight:1.45}}>{e.n&&<span style={{fontWeight:500}}>{e.n} </span>}<span style={{color:C.gray}}>{e.a}</span><div style={{fontSize:10,color:"#8b99a8",marginTop:2}}>{e.t}</div></div></div>);})}
</div>
</div>
</div>
</div>}
...
<SI s={11}/><span style={{fontSize:9,fontWeight:500,color:C.tealDk,letterSpacing:0.5}}>GOAL</span>
<div style={{marginLeft:"auto"}}><Av i={s.owner} s={20} bg={s.owner==="JT"?C.gold:s.owner==="MS"?C.red:C.navy} fg={s.owner==="JT"?C.navy:"#fff"}/></div>
</div>
<div style={{fontSize:14,fontWeight:500,color:C.navy,lineHeight:1.3,marginBottom:8}}>{s.t}</div>
...
<span style={{fontSize:9,fontWeight:500,color:c.type==="project"?C.pinkDk:"#a07509",letterSpacing:0.5}}>{c.type==="project"?"INITIATIVE":"ACTION"}</span>
</div>
<div style={{fontSize:13,fontWeight:500,color:C.navy,lineHeight:1.3,marginBottom:6}}>{c.t}</div>
<Pl c={c.sc}>{c.s}</Pl>
</div>
);})}
</div>
...
<div onClick={function(){setPage("cascade");}} style={{background:C.tealBg,border:"1px solid "+C.tealBd,borderLeft:"3px solid "+C.teal,borderRadius:8,padding:"8px 12px",marginBottom:18,display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}><SI s={12}/><span style={{fontSize:11,fontWeight:500,color:C.tealDk}}>PARENT</span><span style={{fontSize:13,fontWeight:500,color:C.navy}}>Drive enterprise value creation - FY26</span><Pl c="green">{"\u2197"} ON TRACK</Pl></div>
...
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:11,color:C.gray,fontWeight:500}}>{m.l}</span><div style={{display:"flex",alignItems:"center",gap:5}}><Logo k={m.src} s={14}/><span style={{fontSize:10,color:C.gray}}>{m.srcN}</span></div></div>
<div style={{display:"flex",alignItems:"baseline",gap:6,marginBottom:8}}><span style={{fontFamily:"Space Grotesk, sans-serif",fontSize:26,fontWeight:500,color:C.navy,letterSpacing:"-0.02em",lineHeight:1}}>{m.v}</span><span style={{fontSize:10,color:C.gray}}>{"\u2192"} {m.tg}</span></div>
<PB pct={m.p} h={4} color={m.tc}/>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:8}}><Pl c={m.ok?"green":"amber"}>{m.ok?"\u2197":"\u2198"} {m.tr}</Pl><span style={{fontSize:10,color:C.gray}}>{m.p}%</span></div>
<div style={{overflow:"hidden",maxHeight:isExp?80:0,opacity:isExp?1:0,transition:"max-height 0.3s, opacity 0.25s"}}>
<div style={{paddingTop:10,borderTop:"1px solid "+C.subtle,marginTop:10}}><div style={{fontSize:9,color:C.gray,marginBottom:4,letterSpacing:0.5,textTransform:"uppercase",fontWeight:500}}>8-week trend</div>
...
{[["Q1 Margin Analysis","gs","Google Sheets","2d ago"],["Pricing Model v3","conf","Confluence","5d ago"]].map(function(el){return(<div key={el[0]} style={{background:"#fff",border:"1px solid "+C.border,borderRadius:8,padding:"10px 12px",marginBottom:6,cursor:"pointer",display:"flex",alignItems:"center",gap:10}}><Logo k={el[1]} s={20}/><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,color:C.navy,fontWeight:500}}>{el[0]}</div><div style={{fontSize:10,color:C.gray}}>{el[2]} . {el[3]}</div></div></div>);})}
...
<div style={{background:C.navy,padding:"12px 18px",display:"flex",alignItems:"center",gap:10}}><TML s={16}/><span style={{fontSize:11,fontWeight:500,letterSpacing:1,color:"#fff",textTransform:"uppercase"}}>Goal</span></div>
...
<div style={{fontSize:14,color:"#4a5868",lineHeight:1.65,marginBottom:18}}>Drive 300 basis points of gross margin expansion across tier-1 product lines through COGS reduction, price realization, and channel mix optimization.</div>
<div className="trg-owner-row" style={{marginBottom:16}}>
<div style={{background:C.bg2,borderRadius:8,padding:"10px 12px",display:"flex",alignItems:"center",gap:10,minWidth:0}}><Av i="JT" s={30}/><div style={{minWidth:0}}><div style={{fontSize:10,color:C.gray,letterSpacing:0.5,textTransform:"uppercase",fontWeight:500,whiteSpace:"nowrap"}}>Owner</div><div style={{fontSize:13,fontWeight:500,color:C.navy,marginTop:1,whiteSpace:"nowrap"}}>Joe Thompson</div></div></div>
<div style={{background:C.bg2,borderRadius:8,padding:"10px 12px",minWidth:0}}><div style={{fontSize:10,color:C.gray,letterSpacing:0.5,textTransform:"uppercase",fontWeight:500,whiteSpace:"nowrap"}}>Dates</div><div style={{fontSize:13,fontWeight:500,color:C.navy,marginTop:1,whiteSpace:"nowrap"}}>Jan 6 {"\u2192"} Mar 31, 2026</div></div>
</div>
<div className="trg-stat-strip" style={{background:C.bg2,borderRadius:10,marginBottom:16}}>
<div style={{minWidth:0}}><div style={{fontSize:10,color:C.gray,marginBottom:4,letterSpacing:0.5,textTransform:"uppercase",fontWeight:500,whiteSpace:"nowrap"}}>Impact</div><div className="trg-focal-num" style={{fontFamily:"Space Grotesk, sans-serif",fontWeight:500,color:C.navy,letterSpacing:"-0.02em",lineHeight:1,whiteSpace:"nowrap"}}>$24M</div></div>
...
<div style={{padding:"14px 20px",borderBottom:"1px solid "+C.subtle,display:"flex",alignItems:"center",gap:10}}><TM v="a" s={18}/><span style={{fontSize:13,fontWeight:500,color:C.navy}}>Ask TARGA</span><span style={{fontSize:11,color:C.gray,marginLeft:4}}>AI assistant for this goal</span></div>
<div ref={chatRef} style={{minHeight:140,maxHeight:320,overflow:"auto",padding:"18px 24px"}}>
{msgs.length===0&&<div style={{textAlign:"center",padding:"24px 8px",color:"#8b99a8",fontSize:13}}>Ask anything about this goal. Try "What's blocking progress?" or "Who should I talk to about COGS?"</div>}
{msgs.map(function(m,i){return(<div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",marginBottom:10}}><div style={{maxWidth:"78%",padding:"10px 14px",borderRadius:m.role==="user"?"14px 14px 4px 14px":"14px 14px 14px 4px",background:m.role==="user"?C.navy:C.tealBg,color:m.role==="user"?"#fff":C.navy,fontSize:14,lineHeight:1.55}}>{m.text}</div></div>);})}
...
<div style={{padding:"12px 20px 16px",borderTop:"1px solid "+C.subtle,display:"flex",gap:10}}><input value={chatIn} onChange={function(e){setChatIn(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")sendChat();}} placeholder="Ask about this goal..." style={{flex:1,padding:"10px 14px",border:"1px solid "+C.border,borderRadius:8,fontSize:14,color:C.navy,outline:"none",background:C.bg2,fontFamily:"inherit"}}/><div onClick={sendChat} style={{width:42,height:42,background:C.teal,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}><svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M5 12 L19 12 M12 5 L19 12 L12 19" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></div></div>
</div>
...
<div style={{display:"flex",alignItems:"center",gap:5,marginBottom:4}}><PI s={11}/><span style={{fontSize:9,fontWeight:500,color:C.pinkDk}}>INITIATIVE</span></div>
...
{["Comments","Attachments","History","Hierarchy","Relationships"].map(function(t){return(<div key={t} onClick={function(){setBTab(t);}} style={{flex:"1 0 auto",padding:"10px 16px",textAlign:"center",fontSize:11,fontWeight:bTab===t?500:400,color:bTab===t?C.teal:C.gray,borderBottom:bTab===t?"2px solid "+C.teal:"2px solid transparent",cursor:"pointer",transition:"all 0.2s",whiteSpace:"nowrap"}}>{t}</div>);})}
</div>
<div style={{padding:16,minHeight:160}}>
{bTab==="Comments"&&<div>
{[{i:"JT",bg:C.gold,n:"Joe Thompson",t:"2d ago",m:"Make sure pricing is approved before board review. Critical path."},{i:"KM",bg:C.teal,n:"Kyle Moyer",t:"2d ago",m:"Agreed. Finance sync scheduled Thursday."},{i:"MS",bg:C.red,n:"Mark Sternberger",t:"5h ago",m:"Supplier negotiations taking longer. May need to escalate."}].map(function(c,idx){return(<div key={idx} style={{display:"flex",gap:10,marginBottom:18}}><Av i={c.i} bg={c.bg} fg={c.bg===C.gold?C.navy:"#fff"} s={28}/><div style={{flex:1}}><div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:3}}><span style={{fontSize:13,fontWeight:500,color:C.navy}}>{c.n}</span><span style={{fontSize:10,color:"#8b99a8"}}>{c.t}</span></div><div style={{fontSize:14,color:"#4a5868",lineHeight:1.6}}>{c.m}</div></div></div>);})}
<div style={{display:"flex",gap:8,marginTop:12,paddingTop:12,borderTop:"1px solid "+C.subtle}}><div style={{flex:1,background:C.bg2,borderRadius:8,padding:"10px 12px",fontSize:12,color:"#8b99a8"}}>Add a comment...</div><div style={{padding:"10px 16px",background:C.teal,color:"#fff",borderRadius:8,fontSize:13,fontWeight:500,cursor:"pointer"}}>Send</div></div>
</div>}

{bTab==="Attachments"&&<div>
<div className="trg-comments-grid">
{[["Q1 Margin Analysis.xlsx","12 KB . Kyle Moyer . Apr 10"],["Pricing Model v3.pdf","2.4 MB . Joe Thompson . Apr 5"],["Supplier Terms Draft.docx","340 KB . Mark Sternberger . Apr 2"]].map(function(arr){return(<div key={arr[0]} style={{background:C.bg2,borderRadius:8,padding:12,cursor:"pointer"}}><div style={{width:32,height:32,background:C.navyLt,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:8}}><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M6 3 L14 3 L19 8 L19 21 L6 21 Z" stroke={C.navy} strokeWidth="1.5"/><path d="M14 3 L14 8 L19 8" stroke={C.navy} strokeWidth="1.5"/></svg></div><div style={{fontSize:13,fontWeight:500,color:C.navy,marginBottom:3}}>{arr[0]}</div><div style={{fontSize:10,color:C.gray}}>{arr[1]}</div></div>);})}
</div>
...
{page==="dashboard"&&<div>
<PH eyebrow="Executive view" title="Goal Health Dashboard" right={<div style={{fontSize:11,color:C.gray}}>FY26 Q1 . Updated today</div>}/>
...
<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><div style={{width:8,height:8,borderRadius:"50%",background:s.sc==="green"?C.green:C.amber,flexShrink:0}}/><div style={{flex:1,fontSize:15,fontWeight:500,color:C.navy,minWidth:0}}>{s.t}</div><Av i={s.owner} s={20} bg={s.owner==="JT"?C.gold:s.owner==="MS"?C.red:C.navy} fg={s.owner==="JT"?C.navy:"#fff"}/></div>
<div style={{display:"flex",gap:24,marginBottom:8,flexWrap:"wrap"}}><div><div style={{fontSize:9,color:C.gray}}>Progress</div><div style={{fontSize:18,fontWeight:500,color:C.navy}}>{s.progress}%</div></div><div><div style={{fontSize:9,color:C.gray}}>Impact</div><div style={{fontSize:18,fontWeight:500,color:C.navy}}>{s.impact}</div></div><div style={{marginLeft:"auto",display:"flex",gap:4}}><Pl c={s.sc}>{s.status}</Pl>{s.blocker&&<Pl c="red">BLOCKER</Pl>}</div></div>
<PB pct={s.progress}/>
</div>);})}
</div>
<div className="trg-home-side">
...
<div style={{fontSize:13,color:C.navy,lineHeight:1.6,marginBottom:10}}>Three of four plans on track. OpEx dropped 7 confidence points due to vendor blockers. Gross margin has an overdue pricing approval. Recommend escalating both before board review.</div>
...
{boardItems.map(function(b){return(<div key={b.t} style={{marginBottom:14}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:11,fontWeight:500,color:C.navy}}>{b.t}</span><span style={{fontSize:15,fontWeight:500,color:b.status==="green"?C.green:C.red}}>{b.conf}%</span></div><PB pct={b.conf} color={b.status==="green"?C.green:C.red}/><div style={{fontSize:10,color:b.conf>b.prev?C.green:C.red,marginTop:3}}>{b.conf>b.prev?"\u2197":"\u2198"} {Math.abs(b.conf-b.prev)} pts vs last week</div></div>);})}
</div>
</div>
</div>
</div>}
